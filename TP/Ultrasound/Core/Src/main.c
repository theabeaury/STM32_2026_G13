 ...
 reglagecouleur(255, 255, 255);
  ...
  while (1)
  {
	  ...
	  // Passer la broche en sortie
	   ...
	   // Passer la broche en entrée
	   GPIO_InitStruct.Pin  = GPIO_PIN_8;
	   GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	   GPIO_InitStruct.Pull = GPIO_NOPULL;
	   HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

	   // Attendre le passage à l'état haut
	   uint32_t timeout = 0;
	   while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_8) == GPIO_PIN_RESET){
		   delay_ticks(1);
		   if (++timeout>3000000) break;
	   }


	   // Mésurer la durée en état haut
	   timeout = 0;
	   ...
	   // Convertir temps en distance
	   int distance = (time/58);

	   //Affichage
	   char line[17];   // LCD 16 caractères

	   snprintf(line, sizeof(line), "Dist: %4d cm   ", distance);
	   //            1234567890123456
	   // padding d'espaces pour effacer les restes

	   lcd_position(&hi2c1, 0, 0);   // col=0, row=0 (ligne du haut)
	   lcd_print(&hi2c1, line);
